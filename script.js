    ...
    const loadExpenses = (filterStartDateValue = null, filterEndDateValue = null, filterDriverValue = null, filterStoreValue = null) => {
        ...
        expenses.forEach((expense, index) => {
            ...
            if (startDateMatch && endDateMatch && driverMatch && storeMatch) {
                ...
                li.innerHTML = `<h1>${expense.driver}</h1></br> - <p>${expense.store}</p> - R$${expense.amount} - Recebido: R$${expense.received} - Lucro: R$${expense.profit} - ${expense.date} <button onclick="removeExpense(${index})">X</button>`;
